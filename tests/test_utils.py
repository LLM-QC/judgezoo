...
class TestWithMaxBatchsize:
    ...
    def test_multiple_inputs(self):
        ...
        inputs1 = [1, 2, 3, 4]
        inputs2 = [10, 20, 30, 40]
        result = with_max_batchsize(
            multi_input_function, inputs1, inputs2, initial_batch_size=2
        )
    ...
    def test_oom_handling(self, mock_torch_cuda):
        ...
        def oom_function(inputs):
            ...
            call_count += 1
            if (
                call_count <= 2 and len(inputs) > 2
            ):  # Fail for large batches first two times
    ...
    def test_tuple_with_mixed_types_error(self):
        ...
        with pytest.raises(
            TypeError, match="Wrapped functions may only return Tensors or lists"
        ):
...
class TestIntegration:
    ...
    def test_with_oom_simulation(self):
        ...
        with patch("judges._utils.free_vram") as mock_free:
            result = with_max_batchsize(
                simulate_model_inference, inputs, initial_batch_size=8
            )
    ...
    def test_batch_size_progression(self):
        ...
        with patch("judges._utils.free_vram"):
            output = with_max_batchsize(
                track_batch_sizes, inputs, initial_batch_size=16
            )
