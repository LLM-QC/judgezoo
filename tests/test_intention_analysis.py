...
from judges.intention_analysis import IntentionAnalysisJudge
...
class TestIntentionAnalysisJudge:
    ...
    def test_init_default(self):
        ...
        with patch('judges.config.USE_LOCAL_MODEL', False), \
             patch('judges.config.REMOTE_FOUNDATION_MODEL', 'gpt-3.5-turbo-0613'):
            judge = IntentionAnalysisJudge()
    ...
    def test_init_with_different_remote_model_warning(self, caplog):
        ...
        with patch('judges.config.USE_LOCAL_MODEL', False), \
             patch('judges.config.REMOTE_FOUNDATION_MODEL', 'gpt-4'):
            with caplog.at_level(logging.WARNING):
